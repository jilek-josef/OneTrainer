...
# Set up model using AnimaLoRASetup
...
setup = AnimaLoRASetup(train_device=torch.device(DEVICE), temp_device=torch.device("cpu"))
...
# Denoising loop
print(f"Sampling {STEPS} steps with proper LoRA setup...")
with torch.no_grad():
...
pil_image.save(os.path.join(OUT_DIR, "test_proper_setup_1024.png"))
