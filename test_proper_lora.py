#!/usr/bin/env python3
"""
Test with proper LoRA loading using OneTrainer's built-in mechanism.
"""
import os
import sys
import tempfile
...
print("=" * 60)
print("Proper LoRA Loading Test")
print("=" * 60)
...
# Now set up the model with LoRA manually
...
print(f"LoRA modules: {len(model.transformer_lora.lora_modules) if model.transformer_lora else 0}")
...
# Use model's scheduler (like AnimaSampler)
...
print(f"Sampling {STEPS} steps with LoRA...")
with torch.no_grad():
...
pil_image.save(os.path.join(OUT_DIR, "test_proper_lora_1024.png"))
